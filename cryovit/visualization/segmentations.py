...
def _process_file(file_name: str, label_dict: Dict[str, Path], result_dir: Path, threshold: float = 0.5) -> None:
    label_data = {}
    sample = "unknown"
    for label, f_path in label_dict.items():
        ...
        with h5py.File(f_path, 'r') as fh:
            if "data" not in label_data:
                label_data["data"] = fh['data'][()].astype(np.float32) # [0, 1], [D, H, W]
            label_data[label] = fh['predictions'][label][()].astype(np.float32) # [0, 1], [D, H, W]
    # Combine all labels into a single segmentation map
    combined_rgb_seg = np.zeros((*label_data["data"].shape, 3), dtype=np.float32) # [D, H, W, 3]
    for label, seg in label_data.items():
        if label in hue_palette:        
            color = np.array(hue_palette.get(label, (1.0, 1.0, 1.0))).reshape((1, 1, 1, 3)) # default to white if label not in palette
    ...
    final_data = np.stack([label_data["data"], label_data["data"], label_data["data"]], axis=-1) # to RGB
    # Add in data and normalize
    final_rgb = np.where(combined_rgb_seg > threshold, combined_rgb_seg, final_data)
    # Combine and convert to cv2 format
    final_combined = np.concatenate([final_data, final_rgb], axis=2) # [D, H, 2*W, 3]
...
def _process_files(file_dict: Dict[str, Dict[str, Path]], result_dir: Path) -> None:
    result_dir.mkdir(parents=True, exist_ok=True)
    for f_name, labels in file_dict.items():
        print(f"Processing file {f_name}")
        _process_file(f_name, labels, result_dir)

def process_experiment(exp_dir: Path, result_dir: Path, exp_template: str, labels: Optional[List[str]]) -> None:
    result_dir.mkdir(parents=True, exist_ok=True)

    if labels is None:
        exp_names = [d.name for d in exp_dir.iterdir() if d.is_dir() and d.name.startswith(exp_template) and d.name.split("_")[-1] in hue_palette]
        labels = [name.split("_")[-1] for name in exp_names]
    else:
        exp_names = [d.name for d in exp_dir.iterdir() if d.is_dir() and d.name.startswith(exp_template) and d.name.split("_")[-1] in labels]

    files = {}
    for label, exp_name in zip(labels, exp_names):
        file_dir = exp_dir / exp_name
        tomo_files = list(file_dir.glob("**/*.hdf"))
        print(f"Found {len(tomo_files)} .hdf files for label {label} in experiment directory {exp_name}")
    ...
    _process_files(all_files, result_dir / f"{exp_template}_{'_'.join(labels)}_segmentations")
