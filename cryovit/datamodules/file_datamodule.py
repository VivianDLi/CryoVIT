...
class FileDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        data_labels: Optional[Path] = None,
        ...
        **kwargs
        ...
        self.val_files = self._load_files(val_path, val_labels, labels) if val_path is not None else []
    ...
    def _load_files(self, data_path: Path, data_labels: Optional[Path] = None, labels: Optional[List[str]] = None) -> List[FileData]:
        if data_path.is_dir():
            ...
            raise ValueError("Data path must be a directory or a .txt file listing data files.")
        assert len(file_paths) != 0, f"No valid tomogram files found in {data_path}."
        if data_labels is None:
            label_paths = [None] * len(file_paths)
        else:
            if data_labels.is_dir():
                label_paths = sorted([f for f in data_labels.rglob("*") if f.suffix in tomogram_exts])
            elif data_labels.is_file() and data_labels.suffix == ".txt":
                with open(data_labels, "r") as f:
                    label_paths = [Path(line.strip()) for line in f if line.strip()]
            else:
                raise ValueError("Data labels path must be a directory or a .txt file listing label files.")
            assert len(label_paths) != 0, f"No valid label files found in {data_labels}."
            assert len(file_paths) == len(label_paths), "Number of data files must match number of label files."
            file_paths = label_paths
        
        files = []
        for fp, lp in zip(file_paths, label_paths):
            if not fp.exists() or not lp.exists():
                logging.warning(f"File {fp} or label {lp} does not exist, skipping.")
                continue
            files.append(FileData(tomo_path=fp, label_path=lp, sample=fp.parent.name, labels=labels))
        return files
