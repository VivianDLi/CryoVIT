...
def run_training(train_data: Path, train_labels: Path, labels: List[str], model_type: str, model_name: str, label_key: str, result_dir: Path, val_data: Path = None, val_labels: Path = None, log_training: bool = False) -> None:
    ## Setup hydra config
    config_path = Path(__file__).parent / "configs"
    with initialize(version_base="1.2", config_path=str(config_path), job_name="cryovit_train"):
        cfg = compose(config_name="train_model", overrides=[f"name={model_name}", f"label_key={label_key}", f"model={model_type}", "datamodule=file"])
    ...
    datamodule = instantiate(cfg.datamodule, _convert_="all")(data_path=train_data, data_labels=train_labels, labels=labels, val_path=val_data, val_labels=val_labels, dataloader_fn=dataloader_fn, dataset_fn=dataset_fn)
    print("Setup dataset.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("train_labels", type=str, required=True, help="Directory or .txt file of training labels")
    ...
    parser.add_argument("--val_labels", type=str, required=False, help="Directory or .txt file of validation labels")
    ...
    parser.add_argument("--label_key", type=str, required=True, help="Label key to train on.")
    parser.add_argument("--log_training", "-l", action="store_true", help="If set, will log training metrics to TensorBoard.")
    ...
    if val_data is not None:
        ...
        assert val_labels is not None and val_labels.exists(), "Validation labels path does not exist."
    
    run_training(train_data, train_labels, args.labels, model_type, model_name, args.label_key, result_path, val_data, val_labels, log_training=args.log_training)